...
if ( ! class_exists( 'Gravity_Forms_Neoserra_Update_Client_Records_Feed_Add_On' ) ) {
	class Gravity_Forms_Neoserra_Update_Client_Records_Feed_Add_On extends GFFeedAddOn {
		...
		public function init() {
			...
			/**
			 * -------------------------------------
			 * gform_pre_submission with Duplicate Contact Lookup Fix
			 * -------------------------------------
			 */
			add_action( 'gform_pre_submission', function( $form ) {
				...
				// Identify relevant fields (first email & the hidden fields).
				...
				// We'll populate these if we find a matching contact/client.
				$contact_id         = null;
				$contact_has_clients = false;

				// Populate contact if not already set (i.e., if the hidden field is empty).
				if ( $field_ids['neoserra_contact_id'] && empty( $_POST[ 'input_' . $field_ids['neoserra_contact_id'] ] ) ) {

					// Fetch ALL contacts matching this email from NeoSerra.
					$contact_search_response = Crown_Neoserra_Records_Api::get_contacts( array( 'email' => $email ) );

					if (
						is_object( $contact_search_response ) &&
						property_exists( $contact_search_response, 'rows' ) &&
						is_array( $contact_search_response->rows ) &&
						! empty( $contact_search_response->rows )
					) {
						// Loop through each contact to see if it has associated clients.
						foreach ( $contact_search_response->rows as $contact ) {

							// For each contact, check if they have any client(s).
							$client_check = Crown_Neoserra_Records_Api::get_contact_clients( $contact->indivId );
							if (
								is_object( $client_check ) &&
								property_exists( $client_check, 'rows' ) &&
								is_array( $client_check->rows ) &&
								! empty( $client_check->rows )
							) {
								// This contact actually has client records.
								$contact_id          = $contact->indivId;
								$contact_has_clients = true;

								// Populate the hidden field so the rest of the code sees it.
								$_POST[ 'input_' . $field_ids['neoserra_contact_id'] ] = $contact_id;
								break; // stop checking further duplicates
							}
						}
					}
				}

				// Now handle the client ID hidden field. We'll fill it if it's empty.
				$client_ids = array();
				if ( $field_ids['neoserra_client_id'] && empty( $_POST[ 'input_' . $field_ids['neoserra_client_id'] ] ) ) {

					if ( $contact_has_clients && $contact_id ) {
						// We found a contact that definitely has associated clients.
						$client_search_response = Crown_Neoserra_Records_Api::get_contact_clients( $contact_id );
						if (
							is_object( $client_search_response ) &&
							property_exists( $client_search_response, 'rows' ) &&
							is_array( $client_search_response->rows ) &&
							! empty( $client_search_response->rows )
						) {
							$client_ids = array_map( function( $n ) {
								return $n->clientId;
							}, $client_search_response->rows );
						}
					} else {
						// If no contact with clients was found, fall back to old logic:
						// search for client records directly by email.
						...
						$client_ids = (
							...
							? array_map( function( $n ) { return $n->clientId; }, $client_search_response->rows )
							: array();
					}

					// If we found any client(s), store them in the hidden field.
					if ( $client_ids ) {
			...
			/**
			 * End of the changed gform_pre_submission hook
			 */
				...
				if ( $contact_id && ! empty( array_intersect( $client_related_fields, array_keys( $field_indices ) ) ) ) {
					...
					if ( ! empty( $clients ) ) {
						...
						if ( isset( $_GET['client'] ) || count( $client_choices ) == 1 ) {
							if ( array_key_exists( 'neoserra_client_ftEmps', $field_indices ) ) {
								$form['fields'][ $field_indices[ 'neoserra_client_ftEmps' ] ]->defaultValue = $default_client->ftEmps;
							}
							if ( array_key_exists( 'neoserra_client_ptEmps', $field_indices ) ) {
								$form['fields'][ $field_indices[ 'neoserra_client_ptEmps' ] ]->defaultValue = $default_client->ptEmps;
							}
							if ( array_key_exists( 'neoserra_client_grossSales', $field_indices ) ) {
								$form['fields'][ $field_indices[ 'neoserra_client_grossSales' ] ]->defaultValue = $default_client->grossSales;
							}
						}
			...
			add_filter( 'gform_field_choice_markup_pre_render', function( $choice_markup, $choice, $field, $value ) {
				if ( $field->inputName == 'neoserra_client_id' && isset( $choice['isOtherChoice'] ) && boolval( $choice['isOtherChoice'] ) ) {
					return '';
				}
				...
				array(
					...
					'fields' => array(
						...
						array(
							...
							'class' => 'small',
							'tooltip' => esc_html__(
								'When creating a new contact/client record, this is the center ID which it should be assigned (if not defined elsewhere).',
								'gfneoserra'
							),
						),
						array(
		...
		public function process_feed( $feed, $entry, $form ) {
			...
			// If client was newly created, refetch if needed for milestone/capital logs
			...
			if ( $client && ! empty( $milestone_args ) ) {
				if ( $already_in_business && isset( $milestone_args['type'] ) && $milestone_args['type'] == '+BSI' ) {
					// If they're already in business, skip business-start milestone
				} else {
					...
					foreach ( array( 'amount', 'initialAmount' ) as $prop ) {
						if ( array_key_exists( $prop, $milestone_args ) ) {
							$milestone_args[ $prop ] = floatval( preg_replace( '/[^\d\.\-]/', '', $milestone_args[ $prop ] ) );
						}
					}
					foreach ( array( 'date', 'initialDate', 'attribDate' ) as $prop ) {
						if ( array_key_exists( $prop, $milestone_args ) ) {
							$milestone_args[ $prop ] = preg_replace( '/^(\d{4}-\d{2}-\d{2}).*/', '$1', $milestone_args[ $prop ] );
						}
					}
			...
			if ( $client && ! empty( $capital_funding_args ) ) {
				...
				foreach ( array( 'amountReq', 'amountApproved' ) as $prop ) {
					if ( array_key_exists( $prop, $capital_funding_args ) ) {
						$capital_funding_args[ $prop ] = floatval( preg_replace( '/[^\d\.\-]/', '', $capital_funding_args[ $prop ] ) );
					}
				}
				foreach ( array( 'date', 'appdate', 'dateCompleted', 'attribDate' ) as $prop ) {
					if ( array_key_exists( $prop, $capital_funding_args ) ) {
						$capital_funding_args[ $prop ] = preg_replace( '/^(\d{4}-\d{2}-\d{2}).*/', '$1', $capital_funding_args[ $prop ] );
					}
				}

				// Only create a capital record if we have an amountApproved > 0
				...
				// Also handle any "Additional Capital Funding Investments" list
				foreach ( $form['fields'] as $field ) {
			...
			// Optionally send counselor notifications
			if (
				boolval( $feed['meta']['notification_counselor_enabled'] ) &&
				! empty( $counselor_notification_links ) &&
				! empty( $counselor_email )
			) {
